...
(function ($) {
		...
		$ttStPortfItem.each(function(index) {
			...
			if (!$this.is(":last-child")) {
				...
				if (tt_isMobile) {
					...
					tl_ttStPortfItem.to($this, { scale: 0.78, rotateX: "-15deg" });
					tl_ttStPortfItem.to($this.find(".tt-stp-item-image img, .tt-stp-item-video video"), { scale: 1.15 }, 0);
					tl_ttStPortfItem.set($this, { autoAlpha: 0 });
				}
			}
	...
	if ($ttPortfolioSlider.length) { 
		...
		// Add class to the <body> if horizontal direction is enabled
		if ($ttPortfolioSlider.is('.cursor-drag-mouse-down[data-direction="horizontal"]')) {
		...
		if ($ttPortfolioSlider.is("[data-direction]")) {
			...
			var $data_ttPoslDirection = "horizontal"; // by default
		}
		...
		var $ttPortfolioSliderSwiper = new Swiper($ttPortfolioSlider.find(".swiper")[0], {
			...
			pagination: {
				...
				horizontalClass: "tt-posl-pagination-horizontal",
	...
	if ($(window).outerWidth() > 991) {
		$(".tt-sticky-element").each(function() { 
			...
			if ($("#tt-header").is(".tt-header-fixed, .tt-header-scroll")) {
				$ttPinElementOffset = $(".tt-header-inner").innerHeight() + 20;
			} else {
				$ttPinElementOffset = 40;
			}
	...
	// E-mail Ajax Send
	$("#tt-contact-form").submit(function() { // your contact form ID
		var th = $(this);
		$.ajax({
			type: "POST",
	...
	if ($("body").not(".is-mobile").hasClass("tt-magic-cursor")) {
		if ($(window).width() > 1024) {
			...
			// Hide on hover.
			$("a, button, .tt-magnetic-btn, .tt-form-control, .tt-form-radio, .tt-form-check, .tt-hide-cursor, .tt-video, iframe, body.ph-mask-on .ph-caption") // class "hide-cursor" is for global use.
